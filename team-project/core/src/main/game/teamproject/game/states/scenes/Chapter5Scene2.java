...
public class Chapter5Scene2 extends State {
    ...
    public Chapter5Scene2(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/geminibetrayed.jpg"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("As Anya approached the base, she saw them.", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label("Gemini Corporation. They had betrayed her. ", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label("All these years of loyalty gone to waste and \n" + "Gemini now protects the bandidos. ", new Label.LabelStyle(white, Color.WHITE));
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new BattleState(gsm, "boss", "level5"));
        }
    }
