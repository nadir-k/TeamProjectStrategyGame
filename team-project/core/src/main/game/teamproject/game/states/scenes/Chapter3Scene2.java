...
public class Chapter3Scene2 extends State {
    ...
    public Chapter3Scene2(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/zuzannah.jpg"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("Each stone contains a spirit \n" +  "who embodies what they preserve.", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label("Zuzannah is one of the spirits and \n" + "he is responsible for the Stone of Power.", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label("To use this power, one must be bound to it. ", new Label.LabelStyle(white, Color.WHITE));
        ...
        label3.setPosition(AnyaGame.WIDTH/12 - 30, AnyaGame.HEIGHT-450);
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new BattleState(gsm,"zuzzanah","level3"));
        }
    }
