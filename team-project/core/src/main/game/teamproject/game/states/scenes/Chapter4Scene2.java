...
public class Chapter4Scene2 extends State {
    ...
    public Chapter4Scene2(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/ambush.jpg"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("Suddenly, many helicopters with an \n" + "arsenal of soldiers arrived.", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label("They found the stone and \n" +  "they are coming for it in numbers. ", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label("They won’t hesitate to eliminate\n" + "all who pose an obstacle. ", new Label.LabelStyle(white, Color.WHITE));
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new Chapter4Scene3(gsm));
        }
    }
