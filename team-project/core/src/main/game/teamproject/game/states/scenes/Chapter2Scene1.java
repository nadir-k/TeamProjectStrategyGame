...
public class Chapter2Scene1 extends State {
    ...
    public Chapter2Scene1(GameStateManager gsm) {
        super(gsm);
        ...
        bgTex = new Texture(Gdx.files.internal("Scenes/bandits symbol.png"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("Having defeated the forces in the village, ", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label("Anya examines the body \n" + "of one of the soldiers.", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label("She sees a symbol. \n" +  "The symbol of the \n" + "bandidos del caos. ", new Label.LabelStyle(white, Color.WHITE));
        label4 = new Label("An enemy who shows no mercy \n" + "but only ruthlessness.", new Label.LabelStyle(white, Color.WHITE));

        background.setPosition(0f, stage.getHeight() - background.getImageHeight());
        label.setPosition(AnyaGame.WIDTH/12 - 30, AnyaGame.HEIGHT-100);
        label2.setPosition(AnyaGame.WIDTH/12 - 30, AnyaGame.HEIGHT-200);
        label3.setPosition(AnyaGame.WIDTH/12 - 30, AnyaGame.HEIGHT-350);
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new Chapter2Scene2(gsm));
        }
    }
