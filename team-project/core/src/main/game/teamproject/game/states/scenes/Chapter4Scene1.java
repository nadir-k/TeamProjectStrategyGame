...
public class Chapter4Scene1 extends State {
    ...
    public Chapter4Scene1(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/stonepics.png"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("Anya begins to understand that she will always be \n"
                + "forever bound to Zuzannah so she will \n"
                + "never be able to escape the stone.", new Label.LabelStyle(white, Color.WHITE));

        label.setWrap(true);

        background.setPosition(0f, stage.getHeight() - background.getImageHeight());
        label.setPosition(AnyaGame.WIDTH/12 - 30, AnyaGame.HEIGHT-400);

        background.getColor().a = 0f;
        background.addAction(Actions.sequence(Actions.fadeIn(2f), Actions.delay(6f), Actions.fadeOut(2f), Actions.run(new Runnable() {
            @Override
            public void run() {
                finished = true;
            }
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new Chapter4Scene2(gsm));
        }
    }
