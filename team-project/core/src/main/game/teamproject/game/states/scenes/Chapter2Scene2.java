...
public class Chapter2Scene2 extends State {
    ...
    public Chapter2Scene2(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/guardians.jpg"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("As Anya walks in the forest, \n" + "the leaves begin to rustle, \n" + "and the wind blows heavily. ", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label("The call of the mayan hunters occur \n" +  "and many surround her.", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label("She will have to fight to \n" +  "prove her worth of survival. ", new Label.LabelStyle(white, Color.WHITE));
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new BattleState(gsm, "hunter", "level2"));
        }
    }
