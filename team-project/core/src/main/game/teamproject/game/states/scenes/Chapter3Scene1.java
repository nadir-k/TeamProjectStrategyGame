...
public class Chapter3Scene1 extends State {
    ...
    public Chapter3Scene1(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/history.jpg"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label("Before the time of man, came the Lighting.", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label("The Lighting was the creation \n" + "of all life as we know it.", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label("In creating life, 4 stones were created to \n" + "preserve space, time, power and the mind.", new Label.LabelStyle(white, Color.WHITE));
        label4 = new Label("The stone you possess. Is the Stone of Power.", new Label.LabelStyle(white, Color.WHITE));
        ...
        label3.setPosition(AnyaGame.WIDTH/12 - 30, AnyaGame.HEIGHT-400);
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new Chapter3Scene2(gsm));
        }
    }
