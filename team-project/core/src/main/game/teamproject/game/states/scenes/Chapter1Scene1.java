...
public class Chapter1Scene1 extends State {
    ...
    public Chapter1Scene1(GameStateManager gsm) {
        super(gsm);
        viewport = new StretchViewport(AnyaGame.WIDTH, AnyaGame.HEIGHT);
        stage = new Stage (viewport, AnyaGame.batch);
        Gdx.input.setInputProcessor(stage);

        bgTex = new Texture(Gdx.files.internal("Scenes/heli.jpg"));
        background = new Image(bgTex);
        white = new BitmapFont(Gdx.files.internal("Font/white.fnt"), false);

        label = new Label(" Having escaped the ambush, \n Anya begins her journey. ", new Label.LabelStyle(white, Color.WHITE));
        label2 = new Label(" Her means of survival will be to \n reach the camp where a Helicopter \n awaits her arrival. ", new Label.LabelStyle(white, Color.WHITE));
        label3 = new Label(" Her Call of Duty begins… ", new Label.LabelStyle(white, Color.WHITE));
        ...
        label2.setPosition(AnyaGame.WIDTH/12, AnyaGame.HEIGHT-350);
        label3.setPosition(AnyaGame.WIDTH/12, AnyaGame.HEIGHT-425);
    ...
    protected void update(float dt) {
        if(finished){
            gsm.set(new Chapter1Scene2(gsm));
        }
    }
